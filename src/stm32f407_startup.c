...
void DMA2_Stream2_IRQHandler(void)
{
    // Check if TCIF2 is complete
    if (DMA2->LISR & (1U << 21))
    {
        // Clear the flag: CTCIF2
        DMA2->LIFCR |= (1U << 21);

        // Disable SPI1 DMA requests
        SPI1->CR2 &= ~(3U << 0);

        // Pull SPI1 low
        disableCS_SPI1();

        // Swap buffers so main loop gets just-filled data
        swapBuffers();

        // Set flag
        setIMUDataReady(IMU1, 1);
    }
    else if (DMA2->LISR & (1U << 19))
    {
        // Clear error flag
        DMA2->LIFCR |= (1U << 19);
    }
...
void DMA2_Stream3_IRQHandler(void)
{
    // Check if TCIF3 is complete
    if (DMA2->LISR & (1U << 27))
    {
        // Clear flag
        DMA2->LIFCR |= (1U << 27);
    }
    else if (DMA2->LISR & (1U << 25))
    {
        // Clear error flag
        DMA2->LIFCR |= (1U << 25);
    }
...
void DMA1_Stream3_IRQHandler(void)
{
    ...
    // Copy all flags state
    uint32_t temp_flags = DMA1->LISR;

    // Clear all flags
    DMA1->LIFCR = (0x3D << 22);

    // Check if bit TCIF3 is complete
    if (temp_flags & (1U << 27))
    {
        // usartWriteString("DMA1_Stream3_IRQHandler TCIF\n");
...
void DMA1_Stream4_IRQHandler(void)
{
    ...
    // Copy all flags state
    uint32_t temp_flags = DMA1->HISR;

    // Clear all flags
    DMA1->HIFCR = (0x3D << 0);

    // Check if TCIF4 is complete
    if (temp_flags & (1U << 5))
    {
        // usartWriteString("DMA1_Stream4_IRQHandler TCIF\n");
